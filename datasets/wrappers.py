...
class ValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img1, img2, mask = self.dataset[idx]
        mask = self.mask_transform(mask)
        mask = mask_to_onehot(mask,self.dataset.palette)

        return {
            'inp1': self.img_transform(img1),
            'inp2': self.img_transform(img2),
            'gt': mask
        }
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1 = transforms.Resize((self.inp_size, self.inp_size))(img1)
        img2 = transforms.Resize((self.inp_size, self.inp_size))(img2)
