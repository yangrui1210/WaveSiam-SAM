...
def setup(rank, world_size):
    global local_rank
    local_rank = rank  # 设置 global local_rank
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = '5680'
    dist.init_process_group(backend='nccl', init_method='env://', rank=rank, world_size=world_size)
    torch.cuda.set_device(rank)  # 将当前进程绑定到特定 GPU
    print(f"Rank {rank} initialized with GPU {rank}")
...
def make_data_loader(spec, tag=''):
    ...
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
        shuffle=False, num_workers=2, pin_memory=False, sampler=sampler)
    return loader
...
def eval_psnr(loader, model, config):#eval_type=None):
    ...
    if local_rank == 0:
        pbar = tqdm(total=len(loader), leave=False, desc='val', ncols=100)
    else:
        pbar = None
    ...
    for batch in loader:
        ...
        inp2 = batch['inp2']
        # gt = batch['gt']
        input = torch.cat([inp1, inp2], dim=1)

        output_masks = model.infer(input)
        ...
        val_metric1.add(result1.item(), inp1.shape[0])
        val_metric2.add(result2.item(), inp1.shape[0])
        val_metric3.add(result3.item(), inp1.shape[0])
        val_metric4.add(result4.item(), inp1.shape[0])
    if pbar is not None:
        pbar.close()
...
def train(train_loader, model):
    ...
    if local_rank == 0:
        pbar = tqdm(total=len(train_loader), leave=False, desc='train', ncols=100)
    else:
        pbar = None
...
# def main(config_, save_path, args):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        save(config, model, save_path, 'DWT2_'+str(epoch))
        lr_scheduler.step()
...
if __name__ == '__main__':
    ...
    mp.spawn(main, args=(world_size, config, save_path, args), nprocs=world_size, join=True)
